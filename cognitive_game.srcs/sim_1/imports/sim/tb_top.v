...
module tb_top;
    ...
    initial begin
        ...
        $display("==================================================");
        $display("PART C - 3 PLAYER FLOW");
        $display("==================================================");
        
        btnU = 1; #1000; btnU = 0; #25000;
        sw[0] = 1;       // Multi player
        sw[2:1] = 2'b01; // 3 players (1 in num_players_cfg = 3 players)
        #1000;
        
        press_btnC();
        for (trial = 0; trial < 10; trial = trial + 1) begin
            wait_for_led(20); find_led(led, led_idx); #50_000; toggle_sw(led_idx);
            if (trial < 9) wait_for_state(4'd2, 1100); else wait_for_state(4'd14, 10);
        end
        press_btnC();
        for (trial = 0; trial < 10; trial = trial + 1) begin
            wait_for_led(20); find_led(led, led_idx); #50_000; toggle_sw(led_idx);
        ...
        for (trial = 0; trial < 10; trial = trial + 1) begin
            wait_for_led(20); find_led(led, led_idx); #50_000; toggle_sw(led_idx);
            if (trial < 9) wait_for_state(4'd2, 1100); else wait_for_state(4'd12, 10);
        end

        #25000; 
        press_btnR(); #1000; // P1 RT
        press_btnR(); #1000; // P1 ACC
        press_btnR(); #1000; // P2 RT
        press_btnR(); #1000; // P2 ACC
        press_btnR(); #1000;
        if (uut.u_fsm.disp_mode == 3'd1 && uut.display_player_id == 2) $display("[$time] PASS: btnR -> Player 3 RT");
        else $display("[$time] FAIL: disp_mode=%d, pid=%d", uut.u_fsm.disp_mode, uut.display_player_id);
        press_btnR(); #1000;
        if (uut.u_fsm.disp_mode == 3'd2 && uut.display_player_id == 2) $display("[$time] PASS: btnR -> Player 3 ACC");
        else $display("[$time] FAIL: disp_mode=%d, pid=%d", uut.u_fsm.disp_mode, uut.display_player_id);
        press_btnR(); #1000;
        if (uut.u_fsm.disp_mode == 3'd4) $display("[$time] PASS: btnR -> Wraps to DISP_BEST");
        else $display("[$time] FAIL: Wrap failed");
        
        press_btnC(); #1000;
        $display("[$time] 3 PLAYER TEST PASSED!");

        
        $display("==================================================");
        $display("PART D - 4 PLAYER FLOW");
        $display("==================================================");
        
        btnU = 1; #1000; btnU = 0; #25000;
        sw[0] = 1;       // Multi player
        sw[2:1] = 2'b10; // 4 players
        #1000;
        
        press_btnC();
        for (trial = 0; trial < 10; trial = trial + 1) begin
            wait_for_led(20); find_led(led, led_idx); #20_000; toggle_sw(led_idx);
            if (trial < 9) wait_for_state(4'd2, 1100); else wait_for_state(4'd14, 10);
        end
        press_btnC();
        for (trial = 0; trial < 10; trial = trial + 1) begin
        ...
        #25000; 
        press_btnL(); #1000;
        if (uut.u_fsm.disp_mode == 3'd2 && uut.display_player_id == 3) $display("[$time] PASS: btnL -> Wraps to Player 4 ACC");
        else $display("[$time] FAIL: Reverse wrap to P4 ACC failed: %d, pid: %d", uut.u_fsm.disp_mode, uut.display_player_id);
        
        press_btnC(); #1000;
        $display("[$time] 4 PLAYER TEST PASSED!");
